...
def main() -> None:
    ...
    parser.add_argument(
        "-s", "--show-python", help="show python code generated", action="store_true"
    )
    parser.add_argument(
        "-i",
        "--import",
        help="module to import eg: -i 'requests' -i 'numpy'",
        default=[],
        action="append",
    )
    parser.add_argument(
        "-d", "--dry-run", help="don't run python code", action="store_true"
    )
    parser.add_argument(
        "-e",
        "--environment",
        help="show variables excluding -i imports in environment",
        action="store_true",
    )
    ...
    if args.show_python or args.dry_run:
        print(python)
    code = "\n".join([get_user_custom_setup(), python])
